...
BUSINESS_TYPE_ALIASES = {
    ...
    "全国品牌": "national_brand",
    "全国品牌型": "national_brand",
    "b2b_factory": "b2b_factory",
    "b2b工厂制造": "b2b_factory",
    "工厂制造": "b2b_factory",
    "b2b线索型": "b2b_factory",
    "hybrid": "hybrid",
...
def normalize_business_type(value: object) -> str:
    raw = str(value or "").strip()
    return BUSINESS_TYPE_ALIASES.get(raw.lower(), raw)
...
def build_plan(profile: Dict) -> Dict[str, object]:
    ...
    business_type = normalize_business_type(profile.get("business_type", "local_b2c"))
    if business_type not in BUSINESS_TYPES:
        accepted = ", ".join(sorted(BUSINESS_TYPES))
        raise ValueError(f"Unsupported business_type: {business_type}. Expected one of: {accepted}")
